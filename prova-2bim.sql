...
# 1 
INSERT into livros (livros, autor,ano_publicacao, disponivel, categoria,isbn, editora,num_paginas, idioma)
values ('As Crônicas de Nárnia', 'C.S.Lewis', 1950, true, 'Fantasia', '978-0064471190', 'harperCollins', 768, 'Francês');

# 2 
update livros set disponivel= FALSE where 
ano_publicacao < 1950;
...
# 4
delete from livros where idiomas =
'Francês' And ano_publicacao < 2000;

# 5 
SELECT * from livros where num_paginas > 650; 

# 6
SELECT categoria, count(*) AS total FROM 
livros group by categoria;
...
# 9
select * from livros order by
ano_publicacao desc;

# 10 
select* from livros where titulos LIKE 'S%' and ano_publicacao between 1975 and 1990